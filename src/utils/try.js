...
let answers = [`God`, `Satan`];
let question = [`Rather be the `, ` then the `, `, obviuosly`];

// const code = `Rather be the God then the Satan, obviuosly`;

// const chopCode = (code, start, end, answers, question) => {
//   return [
//     answers.concat(code.substr(start, end)),
//     [code.substr(0, start), code.substr(end, code.length - 1)],
//   ];
// };

//   let [answers, question] = chopCode(code, 14, 17, [], []);

//   [answers, question] = chopCode(code, 14, 17, answers, question);

const buildWholeCode = (question, answers) =>
  question.reduce(
    (acum, val) => (
      (acum += val + (answers.length ? answers.shift() : "")), acum
    ),
    ""
  );

console.log(buildWholeCode(question, answers));
